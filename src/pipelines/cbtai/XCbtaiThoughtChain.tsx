...
// 动态生成的基础组件
export const XCbtaiThoughtChain = ({ 
  ...
  return (
    <div ref={ref => { if (ref) { connect(drag(ref)); }}}>
        <ThoughtChain
          ...
          prefixCls={ prefixCls }    
          size={ size }    
          items={ items_temp?items_temp:parse_XAgentContext (  "tcItems",  items ) }
         />
    </div>
  );
};
...
//  是否是容器
XCbtaiThoughtChain.isCanvas = false;
      
const XCbtaiThoughtChainSettings = () => {
    ...
    return (
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="是否可折叠">
                    <Switch
                        checked={ props.collapsible }
                        onChange={(checked) => setProp((props ) => (props.collapsible = checked))}
                    />
                </Form.Item>
...
// 组件配置和默认属性
XCbtaiThoughtChain.craft = {
   displayName: "XCbtaiThoughtChain",
   props: {
     disabled:  false ,
   },
    rules: {
   ...
   related: {
    settings: XCbtaiThoughtChainSettings,
   },
};
