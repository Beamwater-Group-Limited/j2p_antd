...
// 动态生成的定制化组件
export const CustomCbtaiEquipInRender = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomCbtaiEquipInRender')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomCbtaiEquipInRender').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomCbtaiEquipInRender', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomCbtaiEquipInRenderSettings = () => {
...
//  是否是容器
CustomCbtaiEquipInRender.isCanvas = true;
// 组件配置和默认属性
CustomCbtaiEquipInRender.craft = {
  displayName: "CustomCbtaiEquipInRender",
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomCbtaiEquipInRenderSettings,
  },
};
