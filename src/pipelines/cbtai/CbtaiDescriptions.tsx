...
import {   v4   } from "uuid";
import {   Form,    Select,    Switch,    Radio,    Checkbox,    Slider,    Input,    Typography,    InputNumber,    Descriptions   } from "antd";
...
// 动态生成的基础组件
export const CbtaiDescriptions = ({
...
//  是否是容器
CbtaiDescriptions.isCanvas = false;

const CbtaiDescriptionsSettings = () => {
    ...
    return (
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="列表的大小">
                    <Radio.Group
                        value={ props.size }
                        onChange={(e) => setProp((props) => (props.size = e.target.value))}
                    >
                        { ["default","middle","small",].map( (option) => (
                            <Radio key={option} value={option}>
                                {option}
                            </Radio>
                        )) }
...
// 组件配置和默认属性
CbtaiDescriptions.craft = {
  displayName: "CbtaiDescriptions",
  props: {
    disabled:  false ,
  },
  related: {
    settings: CbtaiDescriptionsSettings,
  },
};
