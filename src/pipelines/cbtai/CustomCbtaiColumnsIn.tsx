...
// 动态生成的定制化组件
export const CustomCbtaiColumnsIn = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomCbtaiColumnsIn')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomCbtaiColumnsIn').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomCbtaiColumnsIn', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomCbtaiColumnsInSettings = () => {
...
//  是否是容器
CustomCbtaiColumnsIn.isCanvas = true;
// 组件配置和默认属性
CustomCbtaiColumnsIn.craft = {
  displayName: "CustomCbtaiColumnsIn",
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomCbtaiColumnsInSettings,
  },
};
