...
export const CbtaiRadioGroup = ({ 
  ...
  return (
        ...
        ref={ref => { if (ref) { connect(drag(ref)); }}}
          ...
          options={ options_func?options_func:parse_menuItems(  options) }
          >
         {children}
         </Group>
  );
...
//  是否是容器
CbtaiRadioGroup.isCanvas = true;
      
const CbtaiRadioGroupSettings = () => {
    ...
    return (
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="当前选中的值">
                    <Input
                        value={ props.value }
                        onChange={(e) => setProp((props) => (props.value = e.target.value))}
                    />
                </Form.Item>
                <Form.Item label="Radio的类型">
                    <Radio.Group
                        value={ props.optionType }
                        onChange={(e) => setProp((props) => (props.optionType = e.target.value))}
                    >
                        { ["default","button",].map( (option) => (
                            <Radio key={option} value={option}>
                                {option}
                            </Radio>
                        )) }
...
// 组件配置和默认属性
CbtaiRadioGroup.craft = {
  displayName: "CbtaiRadioGroup",
  props: {
    disabled:  false ,
  },
  related: {
    settings: CbtaiRadioGroupSettings,
  },
};
