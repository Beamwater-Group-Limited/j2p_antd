...
import {   v4   } from "uuid";
import {   Form,    Select,    Switch,    Radio,    Checkbox,    Slider,    Input,    Typography,    InputNumber,    Row   } from "antd";
...
//  是否是容器
CbtaiRow.isCanvas = true;

const CbtaiRowSettings = () => {
        ...
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="垂直对齐方式">
                    ...
                    >
                        {  ["top","middle","bottom","stretch",].map( (option) => (
                ...
                <Form.Item label="水平排列方式">
                    ...
                    >
                        {  ["start","center","end","space-between","space-around","space-evenly",].map( (option) => (
                            <Select.Option key={option} value={option}>
                            {option}
                            </Select.Option>
                        )) }
                ...
                <Form.Item label="栅格间隔">
                    <InputNumber
                        value={ props.gutter }
                        onChange={(value) => setProp((props) => (props.gutter = value))}
                    />
                </Form.Item>
            </Form>
        </div>
...
// 组件配置和默认属性
CbtaiRow.craft = {
  displayName: "CbtaiRow",
  props: {
    disabled:  false ,
  },
  related: {
    settings: CbtaiRowSettings,
  },
};
