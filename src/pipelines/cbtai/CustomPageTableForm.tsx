...
// 动态生成的定制化组件
export const CustomPageTableForm = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomPageTableForm')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomPageTableForm').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomPageTableForm', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomPageTableFormSettings = () => {
...
//  是否是容器
CustomPageTableForm.isCanvas = true;
// 组件配置和默认属性
CustomPageTableForm.craft = {
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomPageTableFormSettings,
  },
};
