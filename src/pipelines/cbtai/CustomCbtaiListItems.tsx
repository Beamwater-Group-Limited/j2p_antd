...
// 动态生成的定制化组件
export const CustomCbtaiListItems = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomCbtaiListItems')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomCbtaiListItems').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomCbtaiListItems', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomCbtaiListItemsSettings = () => {
...
//  是否是容器
CustomCbtaiListItems.isCanvas = true;
// 组件配置和默认属性
CustomCbtaiListItems.craft = {
  displayName: "CustomCbtaiListItems",
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomCbtaiListItemsSettings,
  },
};
