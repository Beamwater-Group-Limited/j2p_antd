  ...
  return (
    <div ref={ref => { if (ref) { connect(drag(ref)); }}}>
        <Item
          ...
          name={ name }    
          colon={ colon }    
          rules={ rules_temp?rules_temp:parse_menuItems ( rules) }
          required={ required }    
          >
         {children}
         </Item>
...
//  是否是容器
CbtaiFormItem.isCanvas = true;
      
const CbtaiFormItemSettings = () => {
    ...
    return (
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="校验规则，设置字段的校验逻辑。">
                    <DictItemTree
                        value={ props.rules }
                        defaultProp={  []  }
                        onChange={(value) => {
                            ...
                            setProp((props) => {
                                props.rules = dictValue;
                                props.rules_temp = parse_menuItems(dictValue);
                            });
                        }}
                    />
                </Form.Item>
                <Form.Item label="必填样式设置。如不设置，则会根据校验规则自动生成">
                    <Switch
...
// 组件配置和默认属性
CbtaiFormItem.craft = {
  displayName: "CbtaiFormItem",
  props: {
    disabled:  false ,
  },
  related: {
    settings: CbtaiFormItemSettings,
  },
};
