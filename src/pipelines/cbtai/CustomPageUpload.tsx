...
export const CustomPageUpload = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomPageUpload')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomPageUpload').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomPageUpload', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomPageUploadSettings = () => {
...
//  是否是容器
CustomPageUpload.isCanvas = true;
// 组件配置和默认属性
CustomPageUpload.craft = {
  displayName: "CustomPageUpload",
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomPageUploadSettings,
  },
};
