...
// 动态生成的定制化组件
export const CustomCbtaiListItem = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomCbtaiListItem')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomCbtaiListItem').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomCbtaiListItem', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomCbtaiListItemSettings = () => {
...
//  是否是容器
CustomCbtaiListItem.isCanvas = true;
// 组件配置和默认属性
CustomCbtaiListItem.craft = {
  displayName: "CustomCbtaiListItem",
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomCbtaiListItemSettings,
  },
};
