...
// 动态生成的定制化组件
export const CustomPageTableInTrans = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomPageTableInTrans')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomPageTableInTrans').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomPageTableInTrans', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomPageTableInTransSettings = () => {
...
//  是否是容器
CustomPageTableInTrans.isCanvas = true;
// 组件配置和默认属性
CustomPageTableInTrans.craft = {
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomPageTableInTransSettings,
  },
};
