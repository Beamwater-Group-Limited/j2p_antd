...
import {   v4   } from "uuid";
import {   Form,    Select,    Switch,    Radio,    Checkbox,    Slider,    Input,    Typography,    InputNumber,    Layout   } from "antd";
...
//  是否是容器
CbtaiLayout.isCanvas = true;

const CbtaiLayoutSettings = () => {
    ...
    return (
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="子元素里是否有Sider">
                    <Switch
                        checked={ props.hasSider }
                        onChange={(checked) => setProp((props ) => (props.hasSider = checked))}
                    />
                </Form.Item>
                <Form.Item label="指定样式">
                    <Input
                        value={ props.style }
...
// 组件配置和默认属性
CbtaiLayout.craft = {
  displayName: "CbtaiLayout",
  props: {
    disabled:  false ,
    children:  "确认" ,
  },
  related: {
    settings: CbtaiLayoutSettings,
  },
};
