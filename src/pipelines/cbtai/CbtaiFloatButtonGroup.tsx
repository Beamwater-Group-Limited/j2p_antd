...
// 动态生成的基础组件
export const CbtaiFloatButtonGroup = ({
...
//  是否是容器
CbtaiFloatButtonGroup.isCanvas = false;

const CbtaiFloatButtonGroupSettings = () => {
        ...
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="触发方式">
                    <Radio.Group
                        value={ props.trigger }
                        onChange={(e) => setProp((props) => (props.trigger = e.target.value))}
                    >
                        { ["click","hover",].map( (option) => (
                            <Radio key={option} value={option}>
                                {option}
                            </Radio>
                        )) }
...
// 组件配置和默认属性
CbtaiFloatButtonGroup.craft = {
  displayName: "CbtaiFloatButtonGroup",
  props: {
    disabled:  false ,
  },
  related: {
    settings: CbtaiFloatButtonGroupSettings,
  },
};
