...
// 动态生成的定制化组件
export const CustomCbtaiDeduplicateAction = ({
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        const jsonLocal = sessionStorage.getItem('CustomCbtaiDeduplicateAction')
        if (jsonLocal){
            setJson(JSON.parse(jsonLocal))
        }else{
            getTreeWithDomID(projectConfig.owner_id, 'CustomCbtaiDeduplicateAction').then(v =>{
                setJson(v)
                sessionStorage.setItem('CustomCbtaiDeduplicateAction', JSON.stringify(v))
            })
        }
    }, []);
  return (
...
const CustomCbtaiDeduplicateActionSettings = () => {
...
//  是否是容器
CustomCbtaiDeduplicateAction.isCanvas = true;
// 组件配置和默认属性
CustomCbtaiDeduplicateAction.craft = {
  displayName: "CustomCbtaiDeduplicateAction",
  props: {
    disabled:  false ,
  },
  related: {
    settings:  CustomCbtaiDeduplicateActionSettings,
  },
};
