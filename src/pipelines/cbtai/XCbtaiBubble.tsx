  ...
  return (
    <div ref={ref => { if (ref) { connect(drag(ref)); }}}>
        <Bubble
          ...
          avatar={ avatar_temp?avatar_temp:parse_icon ( avatar ) }
          placement={ placement }    
          variant={ variant }    
          >
         {children}
...
//  是否是容器
XCbtaiBubble.isCanvas = true;
      
const XCbtaiBubbleSettings = () => {
    ...
    return (
        <div>
            <Form labelCol={{ span:24 }} wrapperCol={{ span:24 }}>
                ...
                <Form.Item label="展示头像">
                    <Input
                        value={ props.avatar }
                        onChange={(e) => {
                            setProp((props) => (props.avatar = e.target.value));
                            setProp((props) =>  (props.avatar_temp = parse_icon(e.target.value) ));
                            }
                        }
                    />
                </Form.Item>
                <Form.Item label="信息位置">
                    <Radio.Group
                        value={ props.placement }
                        onChange={(e) => setProp((props) => (props.placement = e.target.value))}
                    >
                ...
                <Form.Item label="气泡样式变体">
                    <Radio.Group
                        value={ props.variant }
                        onChange={(e) => setProp((props) => (props.variant = e.target.value))}
                    >
                        { ["filled","borderless","outlined","shadow",].map( (option) => (
                            <Radio key={option} value={option}>
                                {option}
                            </Radio>
                        )) }
