...
import {
    ...
    ComponentStatesDefinePanel, DevPageFallback,
} from "@/ide";
import {useParams} from "react-router-dom";
import {
    getCompoFromInner, ProjectData,
...
import {CompStatesProvider} from "@/context/CompStatesContext";
...
function ComponentApp() {
    ...
    // 获取节点类型
    const {innerComponent} = useParams<{ innerComponent: string }>(); // 获取路由参数 :innerComponent
    // 以组件为项目名称
    const [projectConfig, setProjectConfig] = useState<ProjectData>({
        project_id: getCompoFromInner(innerComponent),
        name: getCompoFromInner(innerComponent),
        description: getCompoFromInner(innerComponent),
        owner_id: 'cbtai',          // 所有权
        status: "计划中",
    ...
    const [json, setJson] = useState<any>(() => {
        const savedConfig = localStorage.getItem(getCompoFromInner(innerComponent));
    ...
    const onChange = (query: { serialize: () => any }) => {
        ...
        // 存储的条件是，组件正常加载了
        if (resolver[getCompoFromInner(innerComponent)]) {
            serializeComponentTree(appConfig.userID, getCompoFromInner(innerComponent), json).then(() => {
                // 保存 JSON 数据到本地存储
                localStorage.setItem(getCompoFromInner(innerComponent), JSON.stringify(json));
            })
        }
    };
    // 组件解析器加载
    useEffect(() => {
        const fetchResolver = async () => {
            const loadedResolver = await loadComponents(getCompoFromInner(innerComponent));
            ...
            if (Object.keys(loadedResolver).length === 0) {
                ...
                // 更新节点树
                const compTypeTree = createNodeTree("ROOT", getCompoFromInner(innerComponent));
    ...
    return (
        <>
            {(!process.env.REACT_APP_BASE_PREFIX || process.env.REACT_APP_BASE_PREFIX.trim() === '') ? (
                <ProjectProvider defaultProjectConfig={projectConfig}>
                    <ComponentTypeProvider initialCompType={innerComponent}>
                        <CompConfigProvider>
                            <WebSocketProvider>
                                <CompStatesProvider>
                                    <div className="ComponentApp">
                                        <Typography.Title level={5} className="text-center">
                                            组件配置 - 组件类型：{getCompoFromInner(innerComponent)}
                                        </Typography.Title>
                                        <PagesProvider
                                            defaultPages={{
                                                "pageID": getCompoFromInner(innerComponent),
                                                "nodesStated": []
                                            }}>
