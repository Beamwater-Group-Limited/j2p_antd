...
import {
    ComponentConfig, defaultChartCompConfig,
    deleteComponentType,
    fetchConfigForCompo, generateChart,
    generateComponentsIndex, generateDynamicImport, getChartCompoFromInner,
    getComponentNames,
    LOW_USER_NAME,
    syncLocalChartConfig,
    updateComponent,
...
export const ChartTopbar: React.FC = () => {
    // 加载中
    ...
    const confirmReset = () => {
        Modal.confirm({
            ...
            content: `您确定要${componentExists ? "重置" : "创建"}组件"${getChartCompoFromInner(InnerComp)}"吗?`,
            onOk: () => {
                const compconfig = defaultChartCompConfig(InnerComp)
    ...
    const updateIndexDynamicImport = async () => {
        getComponentNames(projectConfig.owner_id).then(r => {
            ...
            // 异常情况
            // if (!newDynamicComp.components.includes(getChartCompoFromInner(InnerComp))) return
            // 正常处理 生成索引代码
    ...
    const updateNewFile = async (initCode: string, configData: ComponentConfig) => {
        try {
            const response = await updateComponent(appConfig.userID, getChartCompoFromInner(InnerComp), initCode);
            const cheng = await updateComponentConfig(appConfig.userID, getChartCompoFromInner(InnerComp), configData);
            console.log("保存组件文件", response);
            console.log("保存组件配置文件", configData);
            message.success(`新建成功，从服务器下载更新${getChartCompoFromInner(InnerComp)}.tsx到本地，再刷新页面`)
        } catch (error) {
    ...
    useEffect(() => {
        ...
        setComponentExists(dynamicComp.components.includes(getChartCompoFromInner(InnerComp)));
    }, [dynamicComp]);
    ...
    useEffect(() => {
        ...
        // 设置子组件
        formSonComp.setFieldsValue({innerComponent: compConfig.innerComponent})
    }, [compConfig]);
    ...
    useEffect(() => {
        ...
        const fetchConfig = async () => {
            const config = await fetchConfigForCompo(appConfig.userID, getChartCompoFromInner(InnerComp))
            console.log("从服务器加载配置文件", config)
            if (!config) return;
            // 动态修改内容，融合本地预设不变的内容 todo 开发时使用 后续关闭
            const syncLocalConfig = syncLocalChartConfig(config)
                    ...
                    >
                        <Form.Item
                            ...
                            <Input
                                placeholder="请指定组件类型"
                                onChange={(e) => {
                                    const innerComp = `${(e.target as HTMLInputElement).value}`
            ...
            <Col>
                <Button
                    ...
                    onClick={() => {
                        Modal.confirm({
                            title: "确认清除",
                            content: `您确定要清除组件"${getChartCompoFromInner(InnerComp)}"吗?`,
                            onOk: () => {
                                deleteComponentType(appConfig.userID, getChartCompoFromInner(InnerComp)).then(
                                    () => updateIndexDynamicImport()
                                )
