...
import {
    ComponentConfig, defaultXCompConfig,
    deleteComponentType,
    fetchConfigForCompo, generateX,
    generateComponentsIndex, generateDynamicImport, getXCompoFromInner,
    getComponentNames,
    LOW_USER_NAME,
    syncLocalXConfig,
    updateComponent,
    ...
    updateDynamicImport,
    USER_NAME, getSonFromInner
} from "@/tools";
import {useNavigate} from "react-router-dom";

export const XTopbar: React.FC = () => {
    // 加载中
    ...
    const confirmReset = () => {
        Modal.confirm({
            ...
            content: `您确定要${componentExists ? "重置" : "创建"}组件"${getXCompoFromInner(InnerComp)}"吗?`,
            onOk: () => {
                const compconfig = defaultXCompConfig(InnerComp,parentComponentName)
    ...
    const updateIndexDynamicImport = async () => {
        getComponentNames(projectConfig.owner_id).then(r => {
            ...
            // 异常情况
            // if (!newDynamicComp.components.includes(getXCompoFromInner(InnerComp))) return
            // 正常处理 生成索引代码
    ...
    const updateNewFile = async (initCode: string, configData: ComponentConfig) => {
        try {
            const response = await updateComponent(appConfig.userID, getXCompoFromInner(InnerComp), initCode);
            const cheng = await updateComponentConfig(appConfig.userID, getXCompoFromInner(InnerComp), configData);
            console.log("保存组件文件", response);
            console.log("保存组件配置文件", configData);
            message.success(`新建成功，从服务器下载更新${getXCompoFromInner(InnerComp)}.tsx到本地，再刷新页面`)
        } catch (error) {
    ...
    // 监听输入值
    useEffect(() => {
        console.log("输入值被初始化",inputValue)
    }, [inputValue]);
    // 监听组件列表配置项，决定组件存在状态的值
    useEffect(() => {
        if (!dynamicComp) return;
        setComponentExists(dynamicComp.components.includes(getXCompoFromInner(InnerComp)));
    }, [dynamicComp]);
    ...
    useEffect(() => {
        ...
        setInputValue(compConfig.sonComponent)
        // 设置子组件
    ...
    useEffect(() => {
        ...
        const fetchConfig = async () => {
            const config = await fetchConfigForCompo(appConfig.userID, getXCompoFromInner(InnerComp))
            console.log("从服务器加载配置文件", config)
            if (!config) return;
            // 动态修改内容，融合本地预设不变的内容 todo 开发时使用 后续关闭
            const syncLocalConfig = syncLocalXConfig(config)
        ...
        <Row className="flex items-center gap-6 px-4 py-2 bg-gray-100 rounded shadow">
            ...
            <Col className="flex-grow">
                <Space>
                        ...
                        <Form.Item name="sonComp">
                            <Space.Compact style={{ width: "100%" }}>
                                ...
                                <Button
                                    type="primary"
                                    onClick={() => {
                                        const innerComp = `${parentComponentName?? ""}${inputValue}`;
                                        console.log("组成的内部组件名称", innerComp);
                                        setInnerCompType(innerComp);
                                        formSonComp.setFieldsValue({ sonComp: inputValue }); // 可选：同步到 Form.Item
                                    }}
                                >
                                    确认
                                </Button>
                    ...
                    <Typography.Text >{`X${USER_NAME}${InnerComp}`}</Typography.Text>
                </Space>
            </Col>
            ...
            <Col>
                <Button
                    ...
                    onClick={() => {
                        Modal.confirm({
                            title: "确认清除",
                            content: `您确定要清除组件"${getXCompoFromInner(InnerComp)}"吗?`,
                            onOk: () => {
                                deleteComponentType(appConfig.userID, getXCompoFromInner(InnerComp)).then(
                                    () => updateIndexDynamicImport()
                                )
